#!/usr/bin/env python3
"""
Canadian chart importer for MusicTrivia.

This version avoids the fragile weekly Billboard scrape. It imports Canadian
number-one songs from Wikipedia year pages, which is much faster and much less
likely to hang:

- 1957–2007: List_of_number-one_singles_of_YEAR_(Canada)
- 2008+:     List_of_Canadian_Hot_100_number-one_singles_of_YEAR

It upserts songs into the existing SQLite database and writes one
song_sources row per unique song with chart_entries equal to weeks at #1.
"""
...
def chart_table_header_info(table):
    rows = table.find_all("tr")
    if not rows:
        return None
    header_cells = rows[0].find_all(["th", "td"])
    headers = [normalize_text(c.get_text(" ", strip=True)).lower() for c in header_cells]
    header_blob = " ".join(headers)
    if "artist" not in header_blob:
        return None
    if not ("song" in header_blob or "title" in header_blob or "single" in header_blob):
        return None
    ...
    for idx, header in enumerate(headers):
        if issue_idx is None and ("issue" in header or header == "date" or "date" in header):
            issue_idx = idx
        if song_idx is None and ("song" in header or "title" in header or "single" in header):
            song_idx = idx
        if artist_idx is None and "artist" in header:
            artist_idx = idx
...
def extract_chart_entries(table, year):
    info = chart_table_header_info(table)
    if not info:
        return []
    ...
    for tr in rows:
        ...
        # Continuation row from rowspan: same song/artist, additional week at #1.
        if len(cells) <= 2 and current is not None:
            current["weeks_at_1"] += 1
            continue

        # Determine cell positions. The common shapes are:
        #   4 columns: Issue date | Song/Title/Single | Artist | Reference
        #   5 columns: No. | Issue date | Song | Artist(s) | Ref.
        #   3 columns: Issue date | Title | Artist
        issue_date = title = artist = None
        if len(cells) >= 5:
            issue_date = cells[1]
            title = cells[2]
            artist = cells[3]
...
def collect_entries(start_year, end_year, verbose=False):
    ...
    for year in range(start_year, end_year + 1):
        ...
        if verbose:
            print(f"loaded {year}: {len(year_entries)} rows")
...
def import_canadian(conn, start_year, end_year, source_name, dry_run=False, limit=None, verbose=False):
    ensure_schema(conn)
    pages_loaded, prepared = collect_entries(start_year, end_year, verbose=verbose)
